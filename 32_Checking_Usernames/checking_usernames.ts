...
let current_users: string[] = [ "Saba UmAr Abbasi", "NatashHa", "MaNo", "Zoya", "Nazz" ];

let new_users: string[] = ["Sania", "Saba UmAr Abbasi", "Aneela", "Shumaila", "zoya"];
...
new_users.forEach(newUsernames => {
    ...
    if(current_users.map(c_users => c_users.toLowerCase()).includes(lowerCase)){
        ...
        console.log(`The username ${newUsernames} is availabe.`);
    }
})
